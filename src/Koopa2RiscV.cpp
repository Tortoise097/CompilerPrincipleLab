...
// 访问 raw program, #1
void Visit(const koopa_raw_program_t &program) {
    if(debug) cout<<"In Visit #1: Visit_raw_prog"<<endl;
    ...
    Visit(program.funcs); //visit global function definitions
...
void Visit(const koopa_raw_function_t &func) {
    if(debug) cout<<"In Visit #3:Visit_funcs"<<endl;
...
// 访问 return 指令
void Visit(const koopa_raw_return_t &i_ret){ //return_instruction
    ...
    cout<<"  li a0, ";
    Visit(i_ret.value); // call Visit #2, print the value
    cout<<"\n  ret"<<endl;
}
